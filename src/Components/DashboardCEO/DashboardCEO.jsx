...
const DashboardCEO = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployeesAndAttendanceAndTasks = async () => {
      try {
        const response = await axios.get(`http://localhost:3306/api/users/company/${user.company_tag}`);
        const employees = response.data;

        const attendanceAndTasksPromises = employees.map(async (employee) => {
          const attendanceResponse = await axios.get(`http://localhost:3306/api/attendance/${employee.id}`);
          const todayAttendance = attendanceResponse.data.find(att => new Date(att.date).toISOString().split('T')[0] === new Date().toISOString().split('T')[0]);

          const tasksResponse = await axios.get(`http://localhost:3306/api/tasks/${employee.id}`);
