  ...
  const Dashboard = () => {
    ...
    useEffect(() => {
      ...
      const fetchAttendance = async () => {
        try {
          ...
          const todaysAttendance = response.data.find(entry => entry.date === todayDate);
    ...
    // Define time limits for enabling/disabling buttons
    const currentHours = currentTime.getHours();
    const isClockInEnabled = isTesting || (!clockedIn && currentHours >= 7 && currentHours < 8);
    const isClockOutEnabled = isTesting || (clockedIn && currentHours >= 15 && currentHours < 23);
      ...
      <div className="dashboard-container">
        ...
        {activeTab === 'Absensi' && (
          <div className="performance">
            ...
            <button
              ...
              disabled={!isTesting && !isClockInEnabled}
              className={!isClockInEnabled ? 'disabled' : ''}
            >
              CLOCK IN
            </button>
            <button 
              onClick={handleClockOut} 
              disabled={!isTesting && !isClockOutEnabled}
              className={!isClockOutEnabled ? 'disabled' : ''}
            >
              CLOCK OUT
            </button>
            </div>
        ...
        {activeTab === 'Tasks' && (
          <div className="tasks">
            ...
            <button className="submit-task-button" onClick={handleTaskSubmit}>Submit Tasks</button>
          </div>
        )}
      </div>
    );
