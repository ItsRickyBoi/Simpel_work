...
axios.defaults.baseURL = 'https://creamysite.my.id/api';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`/tasks/${user.id}`);
    ...
    const fetchAttendance = async () => {
      try {
        const response = await axios.get(`/attendance/${user.id}`);
  ...
  const handleClockOut = async () => {
    ...
    try {
      await axios.put('/attendance/clockout', {
        user_id: user.id,
  ...
  const handleTaskSubmit = async () => {
    try {
      await Promise.all(tasks.map(task => axios.put(`/tasks/${task.id}`, { status: task.status })));
